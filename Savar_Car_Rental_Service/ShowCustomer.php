...
if (mysqli_num_rows($result) > 0) {
    ...
    echo "<table border = '1'>";
    echo "<tr><th>Customer Id</th><th> Name  </th><th>Address</th><th>Gender</th><th>Email</th><th>Password</th><th>Phone</th></tr>";
  
    // Loop through each row in the result set
    while($row = mysqli_fetch_assoc($result)) {
      // Output the data for each row as an HTML table row
      echo "<tr><td>" . $row["Id"] . "</td><td>" . $row["Name"] . "</td><td>" . $row["Address"] . "</td><td>" . $row["Gender"] . "</td><td>" . $row["Email"] . "</td><td>" . $row["Password"] . "</td><td>" . $row["Phone"] . "</td></tr>";
    }
